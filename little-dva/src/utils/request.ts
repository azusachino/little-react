...
declare interface Headers { [key: string]: string }
...
export class Request {
   ...
   */
  static login() {
    if (!this.isLogining) { this.loginReadyPromise = this.onLogin() }
  ...
  static creatRequests(opts: Options | string): () => {} {
    return async (data = {}, method: Methods = 'GET') => {
      const options = this.combineOptions(opts, data, method)
   ...
   * @memberof Request
   */
  static getApiList(reqConfig) {
    if (!Object.keys(reqConfig).length) { return {} }

    Object.keys(reqConfig).forEach((key) => {
      this.apiLists[key] = this.creatRequests(reqConfig[key])
    })
